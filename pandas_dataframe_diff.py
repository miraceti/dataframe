...
def difference(df1,df2):
    """fonction de difference de 2 dataframe avec les mêmes colonnes mais des valeurs différentes sur certaines lignes:
...
# utilisation de drop : df35 = df34[df34.columns.drop['_merge']]
# utilisation de drop : df35 = df34.loc[:, df34.columns.drop('_merge')]
# utilisation de drop : df35 = df34.drop("_merge", axis=1)
# 2 colonnes supprimées : df35 = df34.loc[:,~df34.columns.isin(['pass','_merge'])]
# 2 colonnes supprimées : df35 = df34.drop(['pass','_merge'], axis=1)
# utilisation de difference : df35 = df34[df34.columns.difference(['_merge'])]
print("DF35\n",df35)
print("\n******************************")